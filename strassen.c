...
void normal_matrix_multiplication( int** MatrixA, int** MatrixB, int** MatrixResult, int MatrixSize );
...
//    }
    ...
    // base case - revert to ordinary matrix multiplication
    if (tam <= THRESHOLD) {
      normal_matrix_multiplication(a, b, c, tam);
      return;
    }
    ...
    // other cases are treated here:
        ...
        int** aResult = allocate_real_matrix(newTam, -1);
        int** bResult = allocate_real_matrix(newTam, -1);
...
int** allocate_real_matrix(int tam, int random)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        // initializes the matrix with random values between 0 and 9
        else {
            if (random == 1) {
                for (j = 0; j < m; j++) {
                    v[i][j] = rand() % 10;
...
void normal_matrix_multiplication( int** a, int** b, int** c, int tam )
{
  int i, j, k;
